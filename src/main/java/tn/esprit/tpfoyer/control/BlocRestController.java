...
public class BlocRestController {
    ...
    public List<Bloc> recuperBlocsParNomEtCap(@PathVariable("nb") String nb, @PathVariable("c") long c) {
        return blocService.trouverBlocsParNomEtCap(nb, c);
    }

    @GetMapping("/find-blocs-by-capacity-range")
    public List<Bloc> findBlocsByCapacityRangeAndSort(
            @RequestParam long minCapacity,
            @RequestParam long maxCapacity,
            @RequestParam String sortBy,
            @RequestParam boolean ascending) {
        return blocService.findBlocsByCapacityRangeAndSort(minCapacity, maxCapacity, sortBy, ascending);
    }
}